...
pair<int,pair<int,int> >evnt[2000005];//<x,bdi><y,bdi>,bdi表示建筑物在bd中的位置 
...
int main()
{
    ...
    {
    	bd[bc]={x,h,y,0};
    	evnt[bc<<1]=make_pair(x,make_pair(bc,0));//0左边界,进入 
    	evnt[(bc<<1)+1]=make_pair(y,make_pair(bc,1));//1右边界 ，离开 
    	bc++;
	}
	sort(evnt,evnt+(bc<<1));//按边界排序
	int nowx=0,nowh=0; 
	for(int i=0;i<(bc<<1);i++)//扫描 
	{
		nowx=evnt[i].first;
		if(evnt[i].second.second==0)//建筑物入 
			Ins(evnt[i].second.first);
		else//建筑物出 
			Del(evnt[i].second.first);
		if(i<(bc<<1)-1 && nowx==evnt[i+1].first)//同一个x发生入和出多个事件 
			continue;
